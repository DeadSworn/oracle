...
#define DEFAULT_DAMAGE_START 25.0
#define DEFAULT_DAMAGE_DEFLECT 25.0
#define DEFAULT_SPEED_START 1065.0
#define DEFAULT_SPEED_DEFLECT 300.0
#define DEFAULT_ORBIT_START 0.265
#define DEFAULT_ORBIT_DEFLECT 0.0265
...
public Action:OnDodgeBallGameFrame(Handle:hTimer)
{
	if (g_bRocketIsValid)	HomingRocketThink();
		...
		if (g_iLastDeadTeam == 2)	CreateRocket(g_iSpawnPointsRedEntity, 2);
...
public Action:OnPlayerDeath(Handle:hEvent, String:strEventName[], bool:bDontBroadcast)
	g_iLastDeadTeam = GetClientTeam(GetClientOfUserId(GetEventInt(hEvent, "userid")));
...
{
	if (g_bRocketIsValid && client == GetEntPropEnt(EntRefToEntIndex(g_iRocketEntity), Prop_Send, "m_hOwnerEntity"))
	{
		g_bRocketIsValid = false;
	}
}
public Action:OnPlayerChangeTeam(Handle:hEvent, String:strEventName[], bool:bDontBroadcast)
{
