...
stock void LoadRate()
{
	float TickRate = (1.0/GetTickInterval())/10;
...
public Action OnSetupFinished(Handle hEvent, char[] strEventName, bool bDontBroadcast)
{
	if (BothTeamsPlaying() == true)
	{
		...
		g_hTimerOther = CreateTimer(0.5, Timer_Other, _, TIMER_REPEAT);
		coefRoundInc = 1.0 + coefRoundIncDefault;
...
public Action OnDodgeBallGameFrame(Handle hTimer)
{
	if (g_bRocketIsValid)
	{
		...
		if (iDeflectionCount > g_iRocketDeflections)
		{
			...
			fTurnRate = (ORBIT_START * coefRoundInc) + ORBIT_DEFLECT * iDeflectionCount; if (fTurnRate > 1.0) fTurnRate = 1.0;
...
public Action Timer_Other(Handle hTimer)
{
	SetHudTextParams(-1.0, 2.0, 0.5, 255, 255, 255, 255);
	for (int iClient = 1; iClient <= MaxClients; iClient++)
		if (IsValidClient(iClient) && !IsFakeClient(iClient) && g_bRocketIsValid)
...
public Action OnRoundEnd(Handle hEvent, char[] strEventName, bool bDontBroadcast)
{
	...
	if (g_hTimerOther != null)
	{
		KillTimer(g_hTimerOther);
		g_hTimerOther = null;
	}
	DestroyRocket();
}
