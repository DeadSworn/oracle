...
public OnPluginStart()
{
	...
	CreateTimer(60.0, Stop_Taunt, _, TIMER_REPEAT);
	CloseHandle(conf);
...
public Action:CheckTaunt(client, const String:command[], argc)
{
	...
	{
		ExecuteTaunt(client, itemdef[client]);
		return Plugin_Changed;
	}
	return Plugin_Continue;
}
stock ExecuteTaunt(client, itemdefS)
{
	static Handle:hItem;
	...
	ReplyToCommand(client, "Item index: %i", itemdef[client]);
	ReplyToCommand(client, "Taunt use");
...
public OnPostThink(client)
{
	if (IsValidClient(client))
	{
		if (!TF2_IsPlayerInCondition(client, TFCond_Taunting))
		{
			...
			ReplyToCommand(client, "Taunt remove");
		}
	}
}
