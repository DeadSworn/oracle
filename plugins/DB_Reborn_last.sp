...
int DAMAGE_START = DEFAULT_DAMAGE_START;
int  DAMAGE_DEFLECT = DEFAULT_DAMAGE_DEFLECT;
...
float coefRoundIncDefault = 0.045;
float coefRoundInc;
...
public void OnGameFrame()
{
	if (g_bRocketIsValid)
	{
		...
		if (iDeflectionCount > g_iRocketDeflections)
		{
			...
			if (!bot_status) 
			{
				Set_Record_Speed(iClient, g_iRocketSpeed, iDeflectionCount);
			}
		}
...
public CreateExplosion(int client, int iTeam, float vOrigin[3], float vAngles[3])
{
  ...
  SetEntProp(ent, Prop_Data, "m_iMagnitude", DAMAGE_START + DAMAGE_DEFLECT * g_iRocketDeflections, 4);
